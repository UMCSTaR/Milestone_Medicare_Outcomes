...
#### overall mean milestone ratings less than 7 

```{r mean_lt_7}
# overall mean less than 7 -----
milestone_medicare_pc %>% 
  group_by(mean_lt_7) %>% 
  mutate(n_surgeons = length(unique(id_physician_npi)),
         n_cases = n(),
         ...
         n_readmit = sum(flg_readmit_30d)) %>% 
  ungroup() %>% 
  distinct(mean_lt_7, n_surgeons, n_cases, n_severe_cmp_poa, n_death,
           n_reop, n_readmit) %>% 
  datatable(rownames = F, caption = "mean milestone ratings less than 7")
```

#### Any Milestone ratings less than 7
...
milestone_medicare_pc %>% 
  ...
  mutate(n_surgeons = length(unique(id_physician_npi)),
         ...
         n_readmit = sum(flg_readmit_30d)) %>% 
  ungroup() %>% 
  distinct(ever_less_7_rating, n_surgeons, n_cases, n_severe_cmp_poa, n_death,
           n_reop, n_readmit) %>% 
  datatable(rownames = F, caption = "mean milestone ratings less than 7")
```
...
milestone_medicare_pc %>% 
  ...
  mutate(n_surgeons = length(unique(id_physician_npi)),
         ...
         n_readmit = sum(flg_readmit_30d)) %>% 
  ungroup() %>% 
  distinct(prof_rating_lt7, n_surgeons, n_cases, n_severe_cmp_poa, n_death,
           n_reop, n_readmit) %>% 
  datatable(rownames = F)
...
milestone_medicare_pc %>% 
  group_by(leadership_rating_lt7) %>% 
  mutate(n_surgeons = length(unique(id_physician_npi)),
         n_cases = n(),
         n_severe_cmp_poa = sum(flg_cmp_po_severe_not_poa),
         n_death = sum(flg_death_30d),
         ...
         n_readmit = sum(flg_readmit_30d)) %>% 
  ungroup() %>% 
  distinct(leadership_rating_lt7, n_surgeons, n_cases, n_severe_cmp_poa, n_death,
           n_reop, n_readmit) %>% 
  datatable(rownames = F)
```
...
milestone_medicare_pc %>% 
  ...
  mutate(n_surgeons = length(unique(id_physician_npi)),
         ...
         n_readmit = sum(flg_readmit_30d)) %>% 
  ungroup() %>% 
  distinct(operative_rating_lt7, n_surgeons, n_cases, n_severe_cmp_poa, n_death,
           n_reop, n_readmit) %>% 
  datatable(rownames = F)
```
