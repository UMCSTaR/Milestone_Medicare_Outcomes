...
milestone_medicare_pc %>% 
  ...
  ungroup() %>% 
  distinct(IntResponseValue_mean, n_surgeons, n_cases, n_severe_cmp_poa, rate_severe_cmp_poa, n_death, rate_death,
           n_reop, rate_reop,rate_reop, n_readmit, rate_readmit) %>% 
  arrange(IntResponseValue_mean) %>% 
  rename(overall_mean = IntResponseValue_mean) %>% 
  datatable(rownames = F, options = list(scrollX = T)) %>% 
  formatPercentage(c('rate_severe_cmp_poa', 'rate_death', 'rate_reop','rate_readmit'), 2)
```
...
milestone_medicare_pc %>% 
  ...
  ungroup() %>% 
  distinct(mean_lt_8, n_surgeons, n_cases, n_severe_cmp_poa, rate_severe_cmp_poa, n_death, rate_death,
           n_reop, rate_reop,rate_reop, n_readmit, rate_readmit) %>% 
  datatable(rownames = F, 
            options = list(scrollX = T),
            caption = "mean milestone ratings less than 8") %>% 
  formatPercentage(c('rate_severe_cmp_poa', 'rate_death', 'rate_reop','rate_readmit'), 2)
```
...
milestone_medicare_pc %>% 
  ...
  ungroup() %>% 
  distinct(ever_less_8_rating, n_surgeons, n_cases, n_severe_cmp_poa, rate_severe_cmp_poa, n_death, rate_death,
           n_reop, rate_reop,rate_reop, n_readmit, rate_readmit) %>% 
  datatable(rownames = F, 
            options = list(scrollX = T)) %>% 
  formatPercentage(c('rate_severe_cmp_poa', 'rate_death', 'rate_reop','rate_readmit'), 2)
