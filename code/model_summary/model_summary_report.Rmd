---
title: "Model Summary Reports"
author: "Xilin Chen"
date: "`r Sys.Date()`"
always_allow_html: true
output:
  html_document:
    toc: true
    toc_depth: 1
    toc_float: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = F,                                     # code
  message = F,
  warning = F,
  error = F,
  comment = NA,
  R.options = list(width = 220),
  dev.args = list(bg = 'transparent'),          # viz
  fig.align = 'center',
  cache = F
)

library(tidyverse)
library(tidyext)
library(kableExtra)
library(arsenal)
library(glmmTMB)
library(mixedup)
library(ggeffects)


kable_df <- function(..., digits=2) {
  kable(...,digits = digits, booktabs = T) %>% 
    kable_styling(full_width = F,
                  latex_options = c("striped", "hold_position"))
    # kable_styling(full_width = F)
}



source("/Users/xilinchen/Documents/Repo/Milestone_Medicare_Outcomes/code/functions/fe.r")
source("/Users/xilinchen/Documents/Repo/Milestone_Medicare_Outcomes/code/functions/predictor_visual_from_model.R")

```


```{r load_data}
load("/Volumes/George_Surgeon_Projects/Milestone_vs_Outcomes/milestone_medicare_ratings.rdata")

milestone_medicare_pc = milestone_medicare_ratings %>% 
  filter(proc_Partial_Colectomy==1) %>% 
  mutate(flg_cmp_po_any_not_poa = as.numeric(flg_cmp_po_any_not_poa),
         flg_cmp_po_severe_not_poa = as.numeric(flg_cmp_po_severe_not_poa))
```

# Description

This document uses the end-year milestone evaluations at the last year of residency from ACGME. Milestone rating for each resident is the mean of all questions.

# Whole Data Description

```{r data_desc}
milestone_medicare_pc %>% 
  summarise(n_case = n(),
            n_surgeon = length(unique(id_physician_npi)),
            n_hosp = length(unique(facility_prvnumgrp)),
            POA_severe_complication_rate = sum(flg_cmp_po_severe_not_poa)/n_case,
            severe_complication_rate = sum(flg_cmp_po_severe)/n_case,
            POA_any_complication_rate = sum(flg_cmp_po_any_not_poa)/n_case,
            any_complication_rate = sum(flg_cmp_po_any)/n_case,
            death_rate = sum(flg_death_30d)/n_case,
            readmit = sum(flg_readmit_30d)/n_case,
            reop = sum(flg_util_reop)/n_case) %>% 
  t() %>% 
  kable_df(caption = "Medicare Outcomes Description")
```


## Table1 for model variables

```{r table1, results="asis"}
milestone_medicare_pc = milestone_medicare_pc %>% 
  mutate(
    # patient
    ses_2grp = factor(ses_2grp, labels = c('bot 2 quint', 'top 3 quint')),
    flg_admit_emerg = factor(flg_admit_emerg, labels = c('Not', 'Emergency')),
    flg_male = factor(flg_male, labels = c('Female', 'Male')),
    # case
    open_colectomy = cpt_cd %in% c(44140:44147, 44160),
    lap_colectomy  = cpt_cd %in% 44204:44208,
    colectomy_type = factor(ifelse(cpt_cd %in% c(44140:44147, 44160), 'open', 'lap')),
    # hosp
    facility_clm_yr = facility_clm_yr + 2007,
    hosp_beds_2grp = factor(hosp_beds_2grp, labels = c('<350', '>=350')),
  )


table_one <- tableby( ~ IntResponseValue_mean + prof_rating_mean + operative_rating_mean + factor(ever_less_7_rating) + flg_male + age_at_admit + factor(e_race_wbho_lable) + AHRQ_score + ses_2grp + colectomy_type + factor(facility_clm_yr) + factor(flg_multi_surgeon) + factor(flg_assistant_surgeon)  + hosp_beds_2grp + val_hosp_mcday2inptday_ratio + val_hosp_rn2bed_ratio, data = milestone_medicare_pc)

mylabels <- list(val_hosp_mcday2inptday_ratio = "medicaid/inpatient days ratio", val_hosp_rn2bed_ratio = "Registered nurse to bed ratios")

summary(table_one, labelTranslations = mylabels)
```


\newpage


```{r rating_desc}
# main predictor
predict_term = extract_fixed_effects(results$Par_any_cmp_all_mean) %>%
  slice(2) %>% 
  pull(term)

milestone_medicare_pc %>% 
  ggplot() +
  geom_bar(aes_string(x = predict_term)) +
  theme_classic() 

milestone_medicare_pc %>% 
  num_by(predict_term) %>% 
  kable_df()
```


## observed outcomes 

```{r observed_outcome}
milestone_medicare_pc %>% 
  mutate(IntResponseValue_mean = as.integer(IntResponseValue_mean)) %>% 
  group_by(IntResponseValue_mean) %>% 
  mutate(n_tot = n()) %>% 
  mutate_at(vars('flg_cmp_po_severe_not_poa',
                 'flg_cmp_po_any_not_poa',
                 'flg_readmit_30d',
                 'flg_death_30d',
                 'flg_util_reop'), sum) %>% 
  distinct(IntResponseValue_mean, n_tot, flg_cmp_po_severe_not_poa,
           flg_cmp_po_any_not_poa,
           flg_readmit_30d,
           flg_death_30d,
           flg_util_reop) %>% arrange(IntResponseValue_mean)  %>% 
  kable_df


milestone_medicare_pc %>% 
  group_by(ever_less_7_rating) %>% 
  mutate(n_tot = n()) %>% 
  mutate_at(vars('flg_cmp_po_severe_not_poa',
                 'flg_cmp_po_any_not_poa',
                 'flg_readmit_30d',
                 'flg_death_30d',
                 'flg_util_reop'), sum) %>% 
  distinct(ever_less_7_rating, n_tot, flg_cmp_po_severe_not_poa,
           flg_cmp_po_any_not_poa,
           flg_readmit_30d,
           flg_death_30d,
           flg_util_reop) %>% arrange(ever_less_7_rating)  %>% 
  kable_df
```


\newpage


# Severe Complication (Excluded POA)

```{r severe_cmp}
model_severe_cmp = results$flg_cmp_po_severe_no

fe(model_severe_cmp)

extract_vc(model_severe_cmp, ci_level = 0) %>% 
    kable_df() 


# predicted probability for surgeons
model_vis(results$flg_cmp_po_severe_poa)


```





\newpage

# Any Complication (Exclude POA)

```{r any_cmp}
model_any_cmp = results$flg_cmp_po_any_poa

fe(model_any_cmp)

extract_vc(model_any_cmp, ci_level = 0) %>%
    kable_df()

# predicted probability for surgeons
model_vis(results$flg_cmp_po_any_poa)

```

\newpage


# Readmit 30 days

```{r readmit}
flg_readmit_30d = results$flg_readmit_30d

fe(flg_readmit_30d)

extract_vc(flg_readmit_30d, ci_level = 0) %>%
    kable_df()

# predicted probability for surgeons
model_vis(results$flg_readmit_30d)
```


\newpage


# Death 30 days

```{r death}
flg_death_30d = results$flg_death_30d

fe(flg_death_30d)

extract_vc(flg_death_30d, ci_level = 0) %>%
    kable_df()

# predicted probability for surgeons
model_vis(results$flg_death_30d)

```


\newpage


# Reoperation

```{r reoperation}
flg_util_reop = results$flg_util_reop

fe(flg_util_reop)

extract_vc(flg_util_reop, ci_level = 0) %>%
    kable_df()

# predicted probability for surgeons
model_vis(results$flg_util_reop)

```


\newpage


# Any Complication Exclude Death

```{r complication}
flg_any_but_death = results$flg_any_but_death

fe(flg_any_but_death)

extract_vc(flg_any_but_death, ci_level = 0) %>%
    kable_df()

# predicted probability for surgeons
model_vis(results$flg_any_but_death)

```