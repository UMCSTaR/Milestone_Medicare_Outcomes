...
# load 24 months ------
load("/Volumes/George_Surgeon_Projects/Milestone_vs_Outcomes/model/models_month24_pc_interaction.rdata")
...
all_inter_terms %>% 
  kable(digits = 2, caption = "interaction term for ratings and assitant surgeon") %>%
...
model:
- outcome: severe complication
- rating: milestone overall (0:<8; 1≥8)
- interaction: milestone rating*had_assist_surgeon
...
extract_fixed_effects(sever_cmp_overall_bin8) %>% 
    kable(digits = 2, caption = "binary milestone ratings for severe complications") %>%
    kable_styling(full_width = F)
...
ggplot(data = p$data,
       aes(
         x = as.factor(mean_ge_8),
         y = yvar,
         colour = as.factor(had_assist_surg)
       ))+
  geom_pointrange(aes(ymin = LCL,
                      ymax = UCL), position = position_dodge(width = 0.1)) +
  labs(y = "Predicted Prob",
       x = "Overall rating (0 <8; 1≥8)",
       color = "Had assist surg (0:no, 1:yes)") +
  theme_classic() +
