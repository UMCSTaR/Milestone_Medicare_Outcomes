...
Adhering to the [tidyverse style guide](https://style.tidyverse.org/) would be enough.  You can usually just Ctrl+Shft+A any highlighted code to get some decent formatting (but not always!).

### R Scripts

R Scrips should be broken up with headers created by Ctrl+shft+R:
...
The should be kept as light as possible and generally serve one function - data processing, modeling, or visualization.
...
- Use 'word-word2' instead of snake case, as this presently causes reference issues for some situations.

### Other

Any non-publication work should prefer html output.
...
The `misc_docs` folder contains pertinent non-manuscript documents, for example other articles, outlines, etc.
...
Generic objects are harder to name because they can be anything, but again, the more descriptive the better.  In general, it's not necessary to use `data` prefix/suffix for data objects, otherwise almost everything would have those as part of the name.  However, it can be useful to use something like `model_` if you have a modeling script to do so, because you can then collect them easily as a list with something like `mget`, which then allows mapping of other functions to the model objects simultaneously.  Such an approach might also work for other types of objects, including data objects.
...
I find plotting functions to not be very useful, because they would typically need a dozen or more arguments, which can't be spread over multiple layers without adding even more arguments, and which have almost no defaults that are generalizable.  Even for the exact same type of data, as an example, text changes which require multiple inputs to changed, possibly even some that were not already built into the function, thereby growing the function further.

It is however very useful to create a theme that will take care of various aspects that are generalizable, but even then long labels/titles etc. can easily undermine any defaults.  The gist is, functions should make repetitive actions easier, not be a mere wrapper that's actually more difficult to use. 
...
Functions should be saved in a `functions/` folder.

One should add appropriate documentation for a function. Ctrl+shft+alt+R creates the template for you.
...
- Names with underscores (snake_case) are generally easier to read than other options, e.g. .case/CamelCase, and dots have specific functional meaning in R.
- Image files should be svg
- As sensitive data will often have to be housed non-locally, if at all possible, the original (sensitive) data used for a project with a repo should be in a folder with the same name as the repo.  For example `some-network-drive/repo-template/data`.


## When in doubt

Consult the [tidyverse style guide](https://style.tidyverse.org/).  It's not perfect but provides some guidance.
